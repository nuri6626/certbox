...
type CareerHomeProps = {
  ...
  popularCommunityPosts: CommunityPost[];
  ...
  setScreen: (
    screen: "home" | "upload" | "result" | "detail" | "schedule" | "vault",
  ) => void;
    ...
    <>
      <header className="mb-7 flex items-start justify-between">
        ...
        <button
          ...
          className="rounded-full bg-white px-4 py-2 text-xs font-bold text-gray-500 shadow-sm"
        >
          로그아웃
        </button>
      </header>
      ...
      <section className="mb-8 grid grid-cols-2 gap-4">
        <button
          ...
          className="rounded-[28px] bg-violet-50 p-5 text-left shadow-sm"
        >
        ...
        <button
          ...
          className="rounded-[28px] bg-emerald-50 p-5 text-left shadow-sm"
        >
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-2xl ${colorClass[c]}`}
                  >
                    <span
                  ...
                  <div>
                    <p
                      className={`text-xs font-bold ${c === "orange" ? "text-orange-500" : c === "rose" ? "text-rose-500" : "text-violet-500"}`}
                    >
                      {event.label} 임박
                    </p>
                ...
                <p
                  className={`text-sm font-semibold ${c === "orange" ? "text-orange-500" : c === "rose" ? "text-rose-500" : "text-violet-500"}`}
                >
      ...
      <section className="mb-8">
        <h2 className="mb-3 text-xl font-extrabold text-gray-950">
          내 커리어 요약
        </h2>
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-[26px] bg-violet-50 p-4 text-center shadow-sm">
            <div className="mx-auto flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-violet-500">
              {Icon.folder}
            </div>
        ...
        <div className="rounded-3xl bg-white p-5 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-2 text-center text-sm font-bold text-gray-800">
            {[18, 19, 20, 21, 22, 23, 24].map((day) => (
              <div
                key={day}
                className={`relative rounded-full py-2 ${day === 21 ? "bg-gray-950 text-white" : ""}`}
              >
                {day}
                <span
          ...
          {recentCertificates.map((cert) => (
            ...
            >
              {cert.imageUrl ? (
                ...
                <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-violet-50 text-xs font-bold text-violet-500">
                  CERT
                </div>
              )}
          ...
          {popularCommunityPosts.length === 0 ? (
            ...
            popularCommunityPosts.map((post) => (
              <button
                ...
                className="flex w-full items-center justify-between border-b border-gray-100 pb-3 text-left last:border-0 last:pb-0"
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    {post.category} · ❤️ {post.likes || 0} · 댓글{" "}
                    {post.comments || 0}
                  </p>
                </div>

                <div className="ml-3 flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-50 text-lg">
                  💬
                </div>
              </button>
            ))
